...
function card(b,s){
  const market=b.market==='iran'?'🇮🇷 ایران':'🌍 جهان';
...
fetch('data/news.json',{cache:'no-store'}).then(r=>r.json()).then(data=>{
  ...
  for(const item of items)for(const b of item.brands||[]){
    ...
    seen.set(b.id,{id:b.id,name:b.name||b.id,fa:b.fa||b.name||b.id,market:b.market||((item.market==='iran'||(item.iran_relevance_score||0)>=70)?'iran':'world'),sector:'سایر',watched:false});
  }
